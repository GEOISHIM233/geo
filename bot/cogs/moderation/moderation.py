...
class Moderation(commands.Cog):
  ...
  async def clear(self, ctx: Context, amount: int = None):
      ...
      Usage:
        !clear         - Deletes up to 10,000 messages (clears the whole channel)
        !clear 50      - Deletes exactly 50 messages
      """
      # If no amount is given, clear ALL messages (up to 10,000)
      if amount is None:
          amount = 10000
      ...
      if amount > 10000:
          embed = discord.Embed(
              ...
              description="You can only delete up to 10,000 messages at a time to avoid rate limits.\nRun `!clear` again to delete more.",
              color=self.color
          )
          return await ctx.send(embed=embed)

      # Send a warning before clearing everything if it's a large amount
      warning_msg = None
      if amount >= 1000:
          warning_msg = await ctx.send(f"⚠️ Deleting **{amount}** messages... This may take a few seconds.")

      # Delete the messages (amount + 1 includes the command message itself)
  ...
  async def clear_error(self, ctx: Context, error):
      if isinstance(error, commands.MissingPermissions):
          ...
          embed = discord.Embed(
              title="❌ Invalid Input",
              description="Please provide a valid number. Example: `!clear 50`\nOr just type `!clear` to clear everything.",
              color=self.color
          )
          await ctx.send(embed=embed)

  # ================================
  # ALL YOUR EXISTING COMMANDS BELOW
  # ================================
